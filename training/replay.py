...
REPLAY_SPEC = {
    ...
    "next_state": ((25,), torch.int8),
    "next_valid_actions": ((4,), torch.bool),
    "step": ((), torch.int32),
    "total_steps": ((), torch.int32),
...
def make_batches_from_data(
    ...
    *,
    ...
    state = data["state"]
    assert state.ndim == 2 and state.shape[1] in (16, 25), state.shape
...
class RecordBuffer:
    ...
    def contiguous_result(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # Dynamically determine state width (16 or 25)
        state_width = segments[0][0].shape[1]
        res_state = np.zeros((size, state_width), dtype=np.int8)
...
class ReplayRecorder:
    ...
    def on_prepared(self, game: VecGameMasked, new_indices: np.ndarray):
        """VecRunner callback"""
    ...
    def _new_segment(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        return (
            np.zeros((self.segment_size, 25), dtype=np.int8),
