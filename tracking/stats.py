    ...
    def __init__(
        ...
        *,
        ...
        if valid_actions is None:
            valid_actions = torch.ones_like(param, dtype=torch.bool)
...
def maskd_entropy(
    ...
    p_lop_p = torch.where(dist.valid_actions, p_lop_p, 0)
...
class RollingStatistics:
    ...
    def __repr__(self) -> str:
        ...
        return (
            ...
            f" median={self.median:{number_fmt}},"
...
class TensorStats:
    ...
    def save_csv(self, epoch: int, save_dir: Path):
        with open(save_dir / "stats.csv", "a") as f:
            if epoch == 0:
                # Header
                header = "epoch,"
                for key in self.table.keys():
                    header += f"{key}_mean,{key}_std,{key}_min,{key}_max," 
            ...
            for key in self.table.keys():
                ...
                row += f"{stats.mean},{stats.std},{stats.min},{stats.max},"
