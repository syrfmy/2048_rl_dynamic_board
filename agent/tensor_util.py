...
def new_tensors(
    ...
    *,
    ...
    """
    Efficiently allocate new tensors for replay/adv specs, with option for uninitialized memory.
    - Use torch.empty if empty=True (for speed when caller will overwrite all values).
    - Avoid unnecessary dtype/device conversions.
    """
    factory = torch.empty if empty else torch.zeros
    def make(shape, dtype):
        return factory(batch_shape + shape, dtype=dtype, device=device, pin_memory=pin_memory)
    result = {}
